...
**Behavior:** Every push runs **Build and test**. The **Publish Worker** job runs **only when both secrets are set**. If they are missing, CI still passes and deploy is skipped — use [Workers Builds](https://developers.cloudflare.com/workers/ci-cd/builds/) in the dashboard instead, or add the secrets to enable Actions deploy.
